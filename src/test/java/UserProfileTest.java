...
public class UserProfileTest {
    ...
    private String expected = "В этом разделе вы можете изменить свои персональные данные";
    ...
    public void userProfileFromMainPage(){
        driver.get(PAGE_URL);
        loginUser(mail, password); //логинимся
        mainPage.header.clickAccountButton(); //переходим по кнопке "Личный кабинет"
        String actual = userProfilePage.displayTextUserProfile(); //сохраняем текст из блока
        Assert.assertEquals("Проверяем наличие текста в личном кабинете", expected, actual); //проверяем, что мы в авторизованном личном кабинете
    }

    @Test
    @DisplayName("Переход из личного кабинета в конструктор")
    public void userProfileToConstructorMainPage(){
        driver.get(PAGE_URL);
        loginUser(mail, password); //логинимся
        mainPage.header.clickAccountButton(); //переходим по кнопке "Личный кабинет"
        userProfilePage.displayTextUserProfile(); //убедились, что мы в авторизованном личном кабинете
        userProfilePage.header.clickConstructorButton(); //тык на кнопку "Конструктор" в шапке
        Assert.assertTrue("Должно отобразиться меню конструктора на главной", mainPage.constructorPage.visibilityMenuBlock()); //проверяем, что перешли на главную, где отображается меню конструктора
    }

    @Test
    @DisplayName("Переход из личного кабинета в конструктор по клику на логотип Stellar Burgers")
    public void userProfileToLogoMainPage(){
        driver.get(PAGE_URL);
        loginUser(mail, password); //логинимся
        mainPage.header.clickAccountButton(); //переходим по кнопке "Личный кабинет"
        userProfilePage.displayTextUserProfile(); //убедились, что мы в авторизованном личном кабинете
        userProfilePage.header.clickLogoButton(); //тык на кнопку "логотип Stellar Burgers" в шапке
        Assert.assertTrue("Должно отобразиться меню конструктора на главной", mainPage.constructorPage.visibilityMenuBlock()); //проверяем, что перешли на главную, где отображается меню конструктора
    }
